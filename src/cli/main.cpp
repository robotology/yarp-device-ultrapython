...
int main(int argc, char* argv[])
{
	...
	grabber = client.getGrabber();

	if (argsMap.find("--set") != argsMap.end())
	{
		std::vector<std::string> setArgs = client.splitString(argsMap["--set"], "=");
		...
		{
			std::cout << e.what() << std::endl << "Control codes can be expressed only in integer values." << std::endl;
			return -1;
		}
		...
		if (!result)
		{
			std::cout << "Unable to set control " + std::to_string(controlCode) + "=" + std::to_string(value) << std::endl << ". Check remote yarpdev device." << std::endl;
			return -1;
		}

		std::cout << "-------------------" << std::endl;
		std::cout << "Value for control code " << controlCode << " set to: " << value << std::endl;
		std::cout << "-------------------" << std::endl;
	...
	if (argsMap.find("--get") != argsMap.end())
	{
		...
		if (!result)
		{
			std::cout << "Unable to get control " + std::to_string(controlCode) << std::endl << ". Check remote yarpdev device." << std::endl;
			return -1;
		}
		std::cout << "-------------------" << std::endl;
