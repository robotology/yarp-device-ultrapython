...
TEST(UltraPython, openAll_ok_002)
{
	...
	EXPECT_CALL(*interface, open_c(_, O_RDWR)).Times(1);
	...
	EXPECT_CALL(*interface, udev_device_unref_c(_))
		...
		.WillOnce(Return(new udev_device()));
	int tmp{1};
	EXPECT_CALL(*interface, mmap_c(_, _, _, _, _, _)).WillRepeatedly(Return((void *)&tmp));
...
TEST(UltraPython, openAll_ko_000)
{
	...
	InterfaceFoCApiMock *interface = new InterfaceFoCApiMock();
	UltraPythonCameraHelper helper(interface);
	EXPECT_CALL(*interface, open_c(_, O_RDWR)).WillOnce(Return(-1));
...
TEST(UltraPython, openAll_ko_001)
{
	...
	struct media_entity_desc info2;
	strcpy(info2.name, "PYTHON1300");
	EXPECT_CALL(*interface, ioctl_media_c(_, _, _)).WillOnce(DoAll(SetArgReferee<2>(info1), Return(1))).WillOnce(DoAll(SetArgReferee<2>(info2), Return(1))).WillOnce(Return(-1));
	EXPECT_CALL(*interface, xioctl(_, _, _)).WillRepeatedly(Return(1));
	EXPECT_CALL(*interface, makedev_c(_, _)).WillOnce(Return(1)).WillOnce(Return(1));
	EXPECT_CALL(*interface, udev_device_new_from_devnum_c(_, _, _)).WillOnce(Return(new udev_device())).WillOnce(Return(new udev_device()));
	EXPECT_CALL(*interface, udev_device_get_devnode_c(_)).WillOnce(Return("video1")).WillOnce(Return("video2"));
	EXPECT_CALL(*interface, udev_device_unref_c(_)).WillOnce(Return(new udev_device())).WillOnce(Return(new udev_device()));
	int tmp{1};
	EXPECT_CALL(*interface, mmap_c(_, _, _, _, _, _)).WillRepeatedly(Return((void *)&tmp));
