...
bool USBCameraDriver::getRawBuffer(unsigned char *buff) { return false; }

int USBCameraDriver::getRawBufferSize() { return 0; }

bool USBCameraDriver::getRgbBuffer(unsigned char *buff) { return false; }
...
bool USBCameraDriver::getRgbSupportedConfigurations(
...
//// RGB ///
UltraPythonDriver::UltraPythonDriver() : USBCameraDriver() {
  yCTrace(USBCAMERA);
  pixelType = VOCAB_PIXEL_RGB;
}

UltraPythonDriver::~UltraPythonDriver() { yCTrace(USBCAMERA); }
...
int UltraPythonDriver::width() const { return USBCameraDriver::width(); }

int UltraPythonDriver::height() const { return USBCameraDriver::height(); }
...
USBCameraDriverRaw::~USBCameraDriverRaw() { yCTrace(USBCAMERA); }

bool USBCameraDriverRaw::getImage(
    yarp::sig::ImageOf<yarp::sig::PixelMono> &image) {
  if ((image.width() != _width) || (image.height() != _height)) {
    image.resize(_width, _height);
  }
...
int USBCameraDriverRaw::width() const { return USBCameraDriver::width(); }

int USBCameraDriverRaw::height() const { return USBCameraDriver::height(); }
