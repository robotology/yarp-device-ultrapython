...
public:
  ...
  static constexpr unsigned int hiresImageBufferSize_{
      hiresWidth_ * hiresHeight_ * 3}; // HI RES buffer size
  static constexpr unsigned int lowresImageBufferSize_{
      lowresWidth_ * lowresHeight_ * 3}; // LOW RES buffer size
...
private:
  ...
  uint32_t remapControl(
      uint32_t v4lCtr) const; // Remap different id from the official v4l ones
